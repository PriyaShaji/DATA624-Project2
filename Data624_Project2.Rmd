...
production team have a better understanding of the manufacturing process, the 
...
The data set is historic data containing predictors associated with the pH and is 
...
further suggests that 79% are complete. There are no missingness patterns, and 
...
Further exploration reveals that some variables may be strongly influenced by 
...
forest (RF) missing data algorithms is implemented because this can handle 
...
variable diagnostics are performed on the predictors to identify if they have very few 
...
data. There is a need for a better model. We will focus and select our 
...
8.40. This means that the model identifies factors that lean more to decreasing 
the pH than to increase it. It is also less than the expected value with a 
difference of 0.15.
...
functions are illustrated. There are quite a few interaction terms between multiple 
hinge functions. 
...
are going to evaluate the performance on the test set to see how the model is performing 
...
one of the most used techniques among the data scientists that is why we are 
...
large with any dimensionality. It seems like a wonderful algorithm to use in our 
...
is to combine several models to improve the prediction accuracy in 
learning problems with a numerical target variable. There are three phases of 
...
combine the predictions of sub-models. The 4 sub-models will the best 
...
the training set will be used to break the tie. The remaining 30% of the historical 
...
suited for modeling the data. This model outperforms the other linear and tree-based 
...
it is clear that the predictions are similar to the actual values. The predictions 
...
best model based on all criteria that will be used to predict the pH levels of 
the evaluation set.
